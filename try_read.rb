...
puts "There are #{number_of_products} in my product table."
...
#_customers = Product.where(:price )

# The where method can also take a string argument.
 
#products_with_c_names = Product.where('name LIKE "C%"')
products_with_c_names=Product.where("price < 10")
puts "abc"
puts products_with_c_names.count
#products_with_c_names.each {|x| puts x.name,x.price }
